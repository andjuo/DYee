...
if [ ${#systMode} -eq 0 ] ; then
    echo "doUnfAuxRndStudy.sh analysisIs2D configFile systMode [runMode]"
    exit
fi
...
while [ ${iSeed} -le ${seedMax} ] ; do
    root -l -q -b plotUnfoldingMatrix.C+\(${analysisIs2D},\"${config}\",${runMode},$systMode,\"${rndStudyStr}${iSeed}\"\) | tee log-unfAuxRndStudy-$((${analysisIs2D}+1))D-${runModeStr}-${rndStudyStr}${iSeed}.out
    iSeed=$(( $iSeed + 1 ))
done
