...
#include "../Covariance/CrossSection.hh"
...
//mass value until which do reweighting
{
  ...
  else if (systematicsMode==DYTools::NORMAL_RND)
    std::cout<<"Running script in the NORMAL_RND mode"<<std::endl;
...
//   Bool_t doSave  = false;    // save plots?
  ...
  const bool regularizeFEWZ = true;
  const bool cutZPT100 = true;
  FEWZ_t fewz(useFewzWeights,cutZPT100,regularizeFEWZ);
  ...
  std::vector<double> specWeightsV;
  random.SetSeed(-1);
  gRandom->SetSeed(-1);

  // The random seeds are needed only if we are running this script in systematics mode
  if (systematicsMode==DYTools::NORMAL_RND) {
    specWeightsV.reserve(randomSeedMax);
    escaleV.reserve(randomSeedMax);
    for (int i=0; i<randomSeedMax; ++i) {
    ...
    specWeightsV.reserve(3);
    escaleV.reserve(3);
    for (int i=0; i<3; i++) {
      ElectronEnergyScale *ees= new ElectronEnergyScale(escale);
      escaleV.push_back(ees);
      specWeightsV.push_back(1.+0.05*(i-1));
    }
  }
  else if (systematicsMode==DYTools::ESCALE_STUDY) {
