// 2014.06.08 optimized version to use ScaleFactor_t
...
int PrepareEtEtaIdx(const esfSelectEvent_t &selData, EtEtaIndexer_t &fidx1, EtEtaIndexer_t &fidx2) {
  ...
  //std::cout << "PrepareEtEtaIdx called for " << selData.et_1 << ", " << selData.eta_1 << "; " << selData.et_2 << ", " << selData.eta_2 << "\n";
  //std::cout << "determined fidx1=" << fidx1 << ", fidx2=" << fidx2 << "\n";
  if (!ok) {
...
int PrepareEtEtaIdx(const esfSelectEvent_t &selData, EtEtaIndexer17_t &fidx1, EtEtaIndexer17_t &fidx2) {
  int ok= (fidx1.setEtEta(selData.et_1,selData.eta_1) &&
	   fidx2.setEtEta(selData.et_2,selData.eta_2)) ? 1:0;
  // std::cout << "PrepareEtEtaIdx[v17] called for " << selData.et_1 << ", " << selData.eta_1 << "; " << selData.et_2 << ", " << selData.eta_2 << "\n";
  //std::cout << "determined fidx1=" << fidx1 << ", fidx2=" << fidx2 << "\n";
  if (!ok) {
    //std::cout << "PrepareEtEtaIdx: failing at " << selData.et_1 << ", " << selData.eta_1 << "; " << selData.et_2 << ", " << selData.eta_2 << "\n";
    ok=1;
...
int studyEffCov_SFsystOpt(int analysisIs2D,
		       int debugMode,
  ...
  EtEtaIndexer17_t fidx1good(etBinCount,etaBinCount);
  EtEtaIndexer17_t fidx2good(etBinCount,etaBinCount);

  ScaleFactor_t SF(scaleFactorKind,etBinCount,etaBinCount);
  if (!SF.init(scaleFactorKind,etBinning,etaBinning,nExps)) {
    std::cout << "Failed to prepare the scale factors (ScaleFactor_t)\n";
    return retCodeError;
  }
  ...
  if (debugMode!=-1) {
    ...
    if (1) {
      selectEventsFName="/media/sdb/andriusj/DYee-20140407-ESF-fromBlackhole/root_files_reg/tag_and_probe/DY_j22_19712pb_egamma_Unregressed_energy/eventSF_selectedEvents.root";
      selectEventsFName="/media/external_hd/eventSF_selectedEvents.root";
      HERE("changed selectEventsFName");
    }
	...
	continue;
      ...
      fidxOk=PrepareEtEtaIdx(selData, fidx1good, fidx2good);
      if (!fidxOk) {
	std::cout << "failed to prepare EtEtaIdx[v17]\n";
	//return;
	continue;
      }
      ...
      double scaleFactor = SF.eventScaleFactor(fidx1good,fidx2good);
      /*
      if (scaleFactor != SF.eventScaleFactor(selData)) {
	std::cout << "detected scale factor error\n";
	std::cout << "scaleFactor=" << scaleFactor << ", fast value="
		  << SF.eventScaleFactor(fidx1good,fidx2good) << ", orig value="
		  << findEventScaleFactor(scaleFactorKind,selData) << "\n";
	std::cout << "fidx1good=" << fidx1good << ", fidx2good=" << fidx2good << "\n";
	//std::cout << "SF.matrix : "; SF.scaleFactorM().Print();
	return retCodeError;
      }
      */
	...
	double scaleFactorSmeared =
	  SF.rndEventScaleFactor(iexp,fidx1good,fidx2good);
	/*
	if (scaleFactorSmeared != SF.rndEventScaleFactor(iexp,selData)) {
	  std::cout << "detected RND scale factor error\n";
	  std::cout << "rndScaleFactor=" << scaleFactorSmeared
		    << ", fast value="
		    << SF.rndEventScaleFactor(iexp,selData)
		    << ", orig value="
		<< findEventScaleFactorSmeared(scaleFactorKind,selData,iexp)
		    << "\n";
	  std::cout << "fidx1good=" << fidx1good
		    << ", fidx2good=" << fidx2good << "\n";
	  //std::cout << "SF.matrix : "; SF.scaleFactorM().Print();
	  return retCodeError;
	}
