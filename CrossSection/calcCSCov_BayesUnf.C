...
#error use_RooUnfold has to be define. Macro uses RooUnfoldBayes
#endif

//=== MAIN MACRO =================================================================================================
// Derived on July 10, 2014 to use Bayes unfolding for detector resolution
// correction


int calcCSCov_BayesUnf(int analysisIs2D,
		       TString conf, int nExps=100,
		       DYTools::TCrossSectionKind_t csKind=DYTools::_cs_None,
		       TString outFileExtraTag_UserInput="",
		       int masterFlag=-1,
		       int nBayesIters_user=-1) {
  ...
  int nBayesIters=1; // number of Bayes iterations
  if (nBayesIters_user!=-1) nBayesIters=nBayesIters_user;
  std::cout << "\n nBayesIters=" << nBayesIters << "\n\n";
  ...
  if (1 && (masterFlag!=-1)) {
    ...
    case 1:
      ...
      calc_UnfRnd=1;
      //calc_UnfResidual=1; // Irrelevant for BayesUnf
      master_nExps=1000;
      //master_nExps=100;
  ...
  outputCSDir.ReplaceAll("xsec",Form("xsec_nBayes%d",nBayesIters));
  if (masterFlag!=-1) {
  ...
  resCSName.ReplaceAll("signal","scs");
  ...
  if (res && doCalcUnfCov) {
	...
	for (int iexp=0; res && (iexp<nExps); ++iexp) {
	  ...
	  std::cout << " iexp=" << iexp << "\n";
	  res=Urnd.randomizeMigrationMatrix(*Uref,NULL,2, 1); // 1-forBayesUnf
	  //if (res) res=unfold_reco2true(hpUnf,Urnd,hpSignalYield);
	  if (res) res= doBayesUnfold(hpUnf,Urnd,hpSignalYield,nBayesIters);
	  if (trackRndMatrix) {
	    TString tmp_fname;
	...
	for (int iseed=seedMin; res && (iseed<=seedMax); iseed+=dSeed) {
	  ...
	  if (res) res= doBayesUnfold(hpUnfYield,Urnd,hpYield,nBayesIters);
	  if (res) {
	...
	for (int iseed=seedMin; res && (iseed<=seedMax); iseed+=dSeed) {
	  ...
	  if (res)res=doBayesUnfold(hpUnfYield,Urnd,hpSignalYield,nBayesIters);
	  if (res) {
  ...
  if (res && doCalcEffCov) {
    ...
    res= doBayesUnfold(hpUnf,*detResponse,hpSignalYield,nBayesIters);
    printHisto(hpUnf,5);
  ...
  if (doCalcFSRCov && DYTools::isPreFsrCS(csKind)) {
	...
	for (int iexp=0; res && (iexp<nExps); ++iexp) {
	  ...
	  // For FSR unfolding, Bayes approach is not used
  ...
  if (res && doCalcGlobalCov) {
	...
	if (1) {
      ...
      InputArgs_t inpArgsLocal(SystematicsStudyName(runSystMode),&inpMgrLocal,
			       runSystMode,csKind);
      inpArgsLocal.bayesUnf(inpArgs.bayesUnf());
