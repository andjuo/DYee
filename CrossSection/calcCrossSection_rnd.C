...
int calcCrossSection_rnd(int analysisIs2D,
			 TString conf,
  ...
  TString confFile=conf;
  if (conf == "defaultRemoteRnd") confFile="defaultAdHocRemote";
  if (!inpMgr.Load(confFile)) return retCodeError;
  InputFileMgr_t inpMgrEScale(inpMgr);
  if (conf == "defaultRemoteRnd") {
    TString dir=inpMgr.rootFileBaseDir();
    dir.ReplaceAll("Results-DYee","Results-rnd-studies");
    inpMgr.rootFileBaseDir(dir);
    // remove ESF tag
    inpMgr.addUserKey(std::string("SCALEFACTORTAG"),"");
    // add info on the ESF file
    inpMgr.addUserKey(std::string("ScaleFactorFullFileName"),Form("../../Results-DYee/root_files_reg/constants/DY_j22_19712pb_egamma_Unregressed_energy/scale_factors_asymHLT_%dD.root",DYTools::study2D+1));
    // no PU-reweight for acceptance
    if (systMode==DYTools::PU_RND_STUDY) {
      inpMgr.addUserKey(std::string("SpecFile_acceptance"),Form("../../Results-DYee/root_files_reg/constants/DY_j22_19712pb/acceptance_%dD.root",DYTools::study2D+1));
    }
  }
  ...
  inpArgs.externalSeed(special_case);
...
int calcCrossSection_simple(TString conf,
  ...
  HistoPair2D_t hpPostFsrDet("hpPostFsrDet",hpEffRhoCorrYield);
  if (res) res=hpPostFsrDet.isInitialized();
  if (res) res=saveResult(inpArgs,hpPostFsrDet,"postFsrDet");
...
void initGlobalFileNames(const TriggerSelection &triggers, const TString &tagDirYields, const TString &tagDirConstants, const TString &tagDirScaleFactorConstants, const TString &tagDirXSect, TFsrCorrectionType_t fsrBinByBin, const TString &mainPath="../root_files/");
...
void initGlobalFileNames(const TriggerSelection &triggers, const TString &tagDirYields, const TString &tagDirConstants, const TString &tagDirScaleFactorConstants, const TString &tagDirXSect, TFsrCorrectionType_t fsrBinByBin, const TString &mainPath) {
  ...
  pathYields=mainPath + TString("yields/") + tagDirYields +
    TString("/");
  pathSystematics=mainPath + TString("systematics/") + tagDirConstants +
    TString("/");
  pathConstants= mainPath + TString("constants/") + tagDirConstants +
    TString("/");
  pathScaleFactors= mainPath + TString("constants/") + tagDirScaleFactorConstants + TString("/");
  pathXSect= mainPath + tagDirXSect + TString("/");
