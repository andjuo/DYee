...
TH1D *loadAlexey1D_v2(int bogus, int histoNo);

//=== MAIN MACRO =================================================================================================

int compare2alexeyV2_covErr(int analysisIs2D=0,
			    int bogus=0,
		   ...
		   )
{
  ...
  TH1D* hAlexeyV2=loadAlexey1D_v2(bogus,2);
  if (!hAlexeyV2) return retCodeError;

  TH1D* hDYeeV2=loadAlexey1D_v2(bogus,1);
  if (!hDYeeV2) return retCodeError;
  ...
  */
    ...
    cp->AddHist1D(hAlexey,"Alexey","LPE1",TAttMarker(kViolet,kMultiply,1.2),1,0,1);
    TString label_loc=Form("Alexey (v2,%d)",bogus);
    cp->AddHist1D(hAlexeyV2,label_loc,"LPE1",TAttMarker(2,2,1.2),1,0,1);
    cp->AddHist1D(hDYeeV2,"DYee","LPE1",TAttMarker(kGreen,27,1),1,0,1);
...
TH1D *loadAlexey1D_v2(int bogus, int histoNo) {
  ...
  if (bogus) fname.ReplaceAll(".root","-bogus.root");
  TString fieldName=(histoNo==1) ? "h2PreFsrCS_DYee" : "h2PreFsrCS_Alexey";
  TH2D *h2=LoadHisto2D(fieldName,fname,"",1);
  h2->SetName("h2tmp");
  TH1D *h1=createProfileAuto(h2,1,fieldName,1);
  return h1;
}
