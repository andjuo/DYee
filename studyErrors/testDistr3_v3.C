...
void testDistr3_v3(int the_case=0, double parMean=0., double par1a=0., double par1step=0.) {
  int includeNegVals=0;
  TH1D *h1a=new TH1D("h1","h1",200+10*includeNegVals,-1.*includeNegVals,20.);
  h1a->SetDirectory(0);
  h1a->SetStats(true);
  ...
  switch(the_case) {
  case -1: fncString="TMath::Gaus(x, [0],[1],[2])"; xRangeMin=-20; gaussian=1;
       fncString="TMath::Exp(-0.5*pow((x-[0])/[1],2))";
    break;
  case 0: fncString= "TMath::LogNormal(x, [0], [1], [2])"; break;
  case 1: fncString= "TMath::GammaDist(x, [0], [1], [2])"; break;
  case 2: {
    double a=-1.4;
    double b= 13.2;
    double c=-7.8;
  ...
  for (int ipar=0; ipar<3; ++ipar) {
    ...
    fnc->SetNpx(2000);
    if (!gaussian) fnc->SetParameters(par1,0,parMean);
    else fnc->SetParameters(parMean,par1);
    fncV.push_back(fnc);
  }
  ...
  for (int ipar=0; ipar<3; ++ipar) {
    ...
    std::cout << " mean=" << hV[ipar]->GetMean()
	      << " +- " << hV[ipar]->GetRMS();
    std::cout << ",    rms/mean=" << hV[ipar]->GetRMS()/hV[ipar]->GetMean();
    std::cout << "\n";
  }
  vx.Print();
  //vtest.Print();
  ...
  printSidedErrors(h1a);
  printSidedErrors(h1a,0.);
}
