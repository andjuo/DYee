...
int calcCSCov_errShape(int studyShape40=-1, int studyShape41=-1,
		       int stopAt=-1,
		       TString correctForNegativeValuesStr="0000",
  ...
  TString fnameErrStudy=Form("errStudy_%d_%d_%s_nExps%d.root",
			     studyShape40,studyShape41,
			     outFileExtraTag_UserInput.Data(),
			     nExps);
  TString errStudy_fnc40, errStudy_pars40;
  TString errStudy_fnc41, errStudy_pars41;
  TVectorD errStudyPar40(5), errStudyPar41(5);

  int multiplicity=10; // scale for finer binning
  int correctForNegativeValuesTrueBkg=0;
  int correctForNegativeValuesFakeBkg=0;
  int correctForNegativeValuesSigYield=0;
  int correctForNegativeValuesUnf=0;

  if (correctForNegativeValuesStr.Length()!=4) {
    std::cout <<"wrong length of 'correctForNegativeValuesStr'=<"
	      << correctForNegativeValuesStr << ">: has to be 4\n";
    return retCodeError;
  }
  else {
    const char *c= correctForNegativeValuesStr.Data();
    correctForNegativeValuesTrueBkg=int(c[0]-'0');
    correctForNegativeValuesFakeBkg=int(c[1]-'0');
    correctForNegativeValuesSigYield=int(c[2]-'0');
    correctForNegativeValuesUnf=int(c[3]-'0');
    std::cout << "correction flags : true2eBkg="
	      << correctForNegativeValuesTrueBkg << ", fakeBkg="
	      << correctForNegativeValuesFakeBkg << ", sigYield="
	      << correctForNegativeValuesSigYield << ", unfYield="
	      << correctForNegativeValuesUnf << "\n";
  }
  

  TH1D *hValShape40tot=NULL, *hValShape41tot=NULL;
  ...
  int calc_YieldSystDetailed=1; // take into account true/fake composition
  ...
  if (res && doCalcYieldCov) {
	...
	if (runSystMode==DYTools::ESCALE_STUDY) {
	  ...
	  HistoPair2D_t hpObservedYield("observedYield");
	  HistoPair2D_t hpTrue2eBkg(fieldTrue2e);
	  HistoPair2D_t hpFakeBkg(fieldFake);
	  int checkBinning=1;
	  int loadSyst=1;
	  if (!hpObservedYield.Load(fnameBgSubtracted,checkBinning,"Input",
				    loadSyst) ||
	      !hpTrue2eBkg.Load(fnameBgSubtracted,checkBinning,"Input",loadSyst) ||
	      !hpFakeBkg.Load(fnameBgSubtracted,checkBinning,"Input",loadSyst)) {
	    std::cout << "failed to load observed yield or backgrounds "
		      << "for iSyst=" << iSyst << "\n";
	    return retCodeError;
	  }
	  HistoPair2D_t hpTrue2eBkg_clone("hpTrue2eBkg_clone",hpTrue2eBkg,1);
	  HistoPair2D_t hpFakeBkg_clone("hpFakeBkg_clone",hpFakeBkg,1);

	  // June 18, 2014: For the error study, we are going to use
	  // the observed yield and the background values. So, do not...
	  /*
	  ...
	  */
	  ...
	  rndFncDefV.push_back("gaus");
	  rndFncDefV.push_back("[0]*x*x+[1]*x+[2]");
	  TF1* rndFnc40=NULL, *rndFnc41=NULL;
	  double shift40=0., shift41=0.;
	  double scale40=1., scale41=0.;

	  errStudy_fnc40="Gauss(default)";
	  errStudy_pars40="default";
	  errStudy_fnc41="Gauss(default)";
	  errStudy_pars41="default";

	  if ((studyShape40>=0) || (studyShape41>=0)) {
	    for (int ibin=40; ibin<=41; ++ibin) {
	      int study=(ibin==40) ? studyShape40 : studyShape41;
	      if (study<0) continue;
	      int fncIdx=study/100;
		...
		return retCodeError;
	      ...
	      TString *strFnc=(ibin==40) ? &errStudy_fnc40 : &errStudy_fnc41;
	      TString *strPars=(ibin==40) ? &errStudy_pars40 : &errStudy_pars41;
	      TVectorD *vPar=(ibin==40) ? &errStudyPar40 : &errStudyPar41;
	      double *scaleVal=(ibin==40) ? &scale40 : &scale41;
	      double *meanVal=(ibin==40) ? &shift40 : &shift41;

	      switch(fncIdx) {
	      case 1: *strFnc="LogNormal"; break;
	      case 2: *strFnc="Gamma"; break;
	      case 3: *strFnc="GausFnc"; break;
	      case 4: *strFnc="parabola"; break;
	      default:
		...
		switch(study) {
		  // GammaDistr
		case 200: scale=1.0; mu=1.15; sigma=0.64; (*meanVal)=0.7363; break;
		case 201: scale=1.0; mu=0.64; sigma=1.15; (*meanVal)=0.7363; break;
		  //case 201: scale=1.0; mu=1.; sigma=0.66; (*meanVal)=0.661; break;
		case 202: scale=0.5; mu=0.58; sigma=10.0; (*meanVal)=2.901; break;
		case 203: scale=0.5; mu=0.82; sigma= 5.0; (*meanVal)=2.051; break;
		case 204: scale=0.5; mu=1.833; sigma=1.0; (*meanVal)=0.917; break;

		default:
		  std::cout << "ibin=40 not ready for study=" << study << "\n";
		  return retCodeError;
		}
	      }
	      else {
		// The simulated error if (0 +- errVal). meanVal should be 0
		switch(study) {
		  // GammaDistr
		case 200: scale=1.; mu=0.75; sigma=1.5; (*meanVal)=0*0.75; break;
		case 201: scale=1.; mu=1.56; sigma=1.0; (*meanVal)=0*1.56; break;
		default:
		  std::cout << "ibin=41 not ready for study=" << study << "\n";
		}
	      }
	      TF1 *fnc=new TF1(Form("fRnd_%d_bin%d",study,ibin),
			       rndFncDefV[fncIdx-1].Data(),
			       xRangeMin,xRangeMax);
	      fnc->SetNpx(2000);
	      fnc->SetParameters(mu,center,sigma);
	      if (ibin==40) rndFnc40=fnc; else rndFnc41=fnc;
	      std::cout << "rndFncDefV[" << fncIdx << "] = " << rndFncDefV[fncIdx] << "\n";
	      std::cout << "ibin=" << ibin << ", fnc="; fnc->Print();
	      *scaleVal= scale;
	      (*vPar)[0]=scale; (*vPar)[1]=mu; (*vPar)[2]=center;
	      (*vPar)[3]=sigma; (*vPar)[4]=(*meanVal);
	      (*strPars)=Form("scale=%4.2lf, mu=%5.2lf, center=%4.2lf, sigma=%4.2lf, meanVal=%4.2lf",scale,mu,center,sigma,(*meanVal));
	    }
	  }
	  ...
	  // separate components
	  TH2D *h2RndTrueTot= Clone(hpSignalYield.histo(),"h2RndTrueTot");
	  TH2D* h2RndFakeTot= Clone(hpSignalYield.histo(),"h2RndFakeTot");

	  vecRnd.reserve(nExps);
	  double chkXRangeMin=-6;
	  double chkXRangeMax= 12;
	  int chkNBins=int((chkXRangeMax-chkXRangeMin)*5 * multiplicity);
	  TH1D* hTBValShape40=new TH1D("hTBValShape40","hTBValShape40",chkNBins,chkXRangeMin,chkXRangeMax);
	  TH1D* hTBValShape41=new TH1D("hTBValShape41","hTBValShape41",chkNBins,chkXRangeMin,chkXRangeMax);
	  TH1D* hTBValShape40tot=new TH1D("hTBValShape40tot","hTBValShape40tot",chkNBins,chkXRangeMin,chkXRangeMax);
	  TH1D* hTBValShape41tot=new TH1D("hTBValShape41tot","hTBValShape41tot",chkNBins,chkXRangeMin,chkXRangeMax);
	  hValShape40tot=new TH1D("hValShape40tot","hValShape40tot",60*multiplicity,18-30,18+30);
	  hValShape41tot=new TH1D("hValShape41tot","hValShape41tot",24*multiplicity, 4-16, 4+16);
	  if (!hTBValShape40 || !hTBValShape41) {
	    std::cout << "null valShape\n";
	    return retCodeError;
	  }
	  if (!hTBValShape40tot || !hTBValShape41tot) {
	    std::cout << "null valShape\n";
	    return retCodeError;
	  }
	  if (!hValShape40tot || !hValShape41tot) {
	    std::cout << "null valShape\n";
	    return retCodeError;
	  }
	  hTBValShape40->Sumw2(); hTBValShape41->Sumw2();
	  hTBValShape40tot->Sumw2(); hTBValShape41tot->Sumw2();
	  hValShape40tot->Sumw2(); hValShape41tot->Sumw2();
	  HERE("start randomization");

	  // randomize statistical error of the true background
	  std::vector<TH2D*> h2True2eBkgStatV;
	  h2True2eBkgStatV.reserve(nExps);
	  for (int iexp=0; iexp<nExps; ++iexp) {
	    if (!hpRndTrueStat.randomizeWithinErr(hpTrue2eBkg,0))
	      return retCodeError;

	    if (rndFnc40 || rndFnc41) {
	      for (int ibin=40; ibin<=41; ++ibin) {
		TF1 *rndFnc= (ibin==40) ? rndFnc40 : rndFnc41;
		double scale= (ibin==40) ? scale40 : scale41;
		if (!rndFnc) continue;
		double newVal= scale * rndFnc->GetRandom();
		//std::cout << "ibin=" << ibin << ", newVal=" << newVal << "\n";
		hpRndTrueStat.SetBinContent(ibin,1, newVal);
	      }
	    }
	    TH2D* h2=Clone(hpRndTrueStat.histo(),
			   Form("h2True2eBkgStat_%d",iexp));
	    h2True2eBkgStatV.push_back(h2);
	  }
	  ...
	  shift41=0.;
	  if (shiftMean) {
	    double centralVal40=hpTrue2eBkg.GetBinContent(40,1);
	    double centralVal41=hpTrue2eBkg.GetBinContent(41,1);
	    double meanVal40=0.;
	    double meanVal41=0.;
	    for (int iexp=0; iexp<nExps; ++iexp) {
	      const TH2D* h2= h2True2eBkgStatV[iexp];
	      meanVal40 += h2->GetBinContent(40,1);
	      meanVal41 += h2->GetBinContent(41,1);
	    }
	    meanVal40 /= double(nExps);
	    meanVal41 /= double(nExps);

	    shift40= centralVal40 - meanVal40;
	    shift41= centralVal41 - meanVal41;
	    std::cout << "randomized mean @40 is " << meanVal40
		      << ", expected=" << centralVal40
		      << ", shift=" << shift40 << "\n";
	    std::cout << "randomized mean @41 is " << meanVal41
		      << ", expected=" << centralVal41
		      << ", shift=" << shift41 << " (not applied)\n";
	  }

	  // Bin 40 is (value +- err) . Apply shift
	  // Bin 41 is (0 +- err) . Do not apply shift
	  for (int iexp=0; iexp<nExps; ++iexp) {
	    TH2D* h2= h2True2eBkgStatV[iexp];
	    double val= h2->GetBinContent(40,1);
	    h2->SetBinContent(40,1, val + shift40);
	    hTBValShape40->Fill( val );
	    hTBValShape41->Fill( h2->GetBinContent(41,1) );
	  }


	  // randomize other errors
	  for (int iexp=0; iexp<nExps; ++iexp) {
	    TH2D *hRnd=Clone(hpObservedYield.histo(),Form("hSystRnd_%d",iexp));
	    if (!hRnd ||
	    ...
	    // Add backgrounds
	    if (!Copy(hpTrue2eBkg.histo(), h2RndTrueTot) ||
		!Copy(hpFakeBkg.histo(), h2RndFakeTot)) {
	      std::cout << "error preparing h2Rnd__Tot\n";
	      return retCodeError;
	    }
	    h2RndTrueTot->Scale(-1.); // compensate double central value
	    //h2RndTrueTot->Add(hpRndTrueStat.histo(),1);
	    h2RndTrueTot->Add(h2True2eBkgStatV[iexp], 1.);
	    h2RndTrueTot->Add(hpRndTrueSyst.histo(),1);
	    h2RndFakeTot->Scale(-1.); // compensate double central value
	    h2RndFakeTot->Add(hpRndFakeStat.histo(),1);
	    h2RndFakeTot->Add(hpRndFakeSyst.histo(),1);

	    // correct the background values for negative entries
	    if (correctForNegativeValuesTrueBkg) {
	      correctNegativeValues(h2RndTrueTot);
	    }
	    if (correctForNegativeValuesFakeBkg) {
	      int prn=0;
	      TH2D* h2Store=(!prn) ? NULL : Clone(h2RndFakeTot,"h2Store");
	      int corr_count=correctNegativeValues(h2RndFakeTot);
	      if (prn) {
		if (corr_count) {
		  std::cout << " corrected " << corr_count << " values\n";
		  printHisto(h2Store);
		  printHisto(h2RndFakeTot);
		}
		delete h2Store;
	      }
	    }

	    hTBValShape40tot->Fill(h2RndTrueTot->GetBinContent(40,1));
	    hTBValShape41tot->Fill(h2RndTrueTot->GetBinContent(41,1));

	    hRnd->Add(h2RndTrueTot,-1.);
	    hRnd->Add(h2RndFakeTot,-1.);

	    if (correctForNegativeValuesSigYield) {
	      correctNegativeValues(hRnd);
	    }

	    hValShape40tot->Fill(hRnd->GetBinContent(40,1));
	    hValShape41tot->Fill(hRnd->GetBinContent(41,1));
	  ...
	  if (1) {
	    ...
	    ComparisonPlot_t cp("cp","spec true2e bkg","events","count","ratio");
	    cp.SetRefIdx(-111);
	    cp.AddHist1D(hTBValShape40,"true2e bkg @40","LP", TAttMarker(kBlack,24,1.),1,0,1);
	    cp.AddHist1D(hTBValShape41,"true2e bkg @41","LP", TAttMarker(kBlue,24,1.),1,0,1);
	    cp.Draw1(cx);

	    TCanvas *cy=new TCanvas("cy","cy",600,600);
	    ComparisonPlot_t cp2("cp2","tot bkg distributions","events","count","ratio");
	    cp2.SetRefIdx(-111);
	    removeError(hTBValShape40tot);
	    removeError(hTBValShape41tot);
	    cp2.AddHist1D(hTBValShape40tot,"bkg @40","LP", TAttMarker(kBlack,24,1.),1,0,1);
	    cp2.AddHist1D(hTBValShape41tot,"bkg @41","LP", TAttMarker(kBlue,24,1.),1,0,1);
	    cp2.Draw1(cy);

	    if (1) {
	      std::cout << "TBValShape40 info: mean="
			<< hTBValShape40->GetMean() << " +- "
			<< hTBValShape40->GetRMS() << "\n";
	      std::cout << "TBValShape41 info: mean="
			<< hTBValShape41->GetMean() << " +- "
			<< hTBValShape41->GetRMS() << "\n";

	      #ifdef studyHelpers_HH
	      double errPos=0,errNeg=0, rms=0.;
	      calculateSidedErrors(hTBValShape40,
				   hTBValShape40->GetMean(),errPos,errNeg,rms);
	      std::cout << "calculateSidedErrors derived errors @40: +"
			<< errPos << ", -" << errNeg << "; rms=" << rms <<"\n";
	      calculateSidedErrors(hTBValShape41,
				   hTBValShape41->GetMean(),errPos,errNeg,rms);
	      std::cout << "calculateSidedErrors derived errors @41 (from "
			<< Form("%4.2lf",hTBValShape41->GetMean()) << ") : +"
			<< errPos << ", -" << errNeg << "; rms=" << rms <<"\n";
	      calculateSidedErrors(hTBValShape41,
				   0,errPos,errNeg,rms);
	      std::cout << "calculateSidedErrors derived errors @41 (from zero): +"
			<< errPos << ", -" << errNeg << "; rms=" << rms <<"\n";
	      #endif
	    }
	    if (1) {
	      std::cout << "TBValShape40tot info: mean="
			<< hTBValShape40tot->GetMean() << " +- "
			<< hTBValShape40tot->GetRMS() << "\n";
	      std::cout << "TBValShape41tot info: mean="
			<< hTBValShape41tot->GetMean() << " +- "
			<< hTBValShape41tot->GetRMS() << "\n";

	      #ifdef studyHelpers_HH
	      double errPos=0,errNeg=0, rms=0;
	      calculateSidedErrors(hTBValShape40tot,
				   hTBValShape40tot->GetMean(),
				   errPos,errNeg,rms);
	      std::cout << "calculateSidedErrors derived errors @40: +"
			<< errPos << ", -" << errNeg << "; rms=" << rms <<"\n";
	      calculateSidedErrors(hTBValShape41tot,
				   hTBValShape41tot->GetMean(),
				   errPos,errNeg,rms);
	      std::cout << "calculateSidedErrors derived errors @41: +"
			<< errPos << ", -" << errNeg << "; rms=" << rms <<"\n";
	      #endif
	    }

	    TCanvas *cz=new TCanvas("cz","cz",600,600);
	    ComparisonPlot_t cp3("cp3","signal yield @40","events","count","ratio");
	    cp3.SetRefIdx(-111);
	    removeError(hValShape40tot);
	    cp3.AddHist1D(hValShape40tot,"sig @40","LP", TAttMarker(kBlack,24,1.),1,0,1);
	    cp3.Draw1(cz);

	    TCanvas *ct=new TCanvas("ct","ct",600,600);
	    ComparisonPlot_t cp4("cp4","signal yield @41","events","count","ratio");
	    cp4.SetRefIdx(-111);
	    removeError(hValShape41tot);
	    cp4.AddHist1D(hValShape41tot,"sig @41","LP", TAttMarker(kBlue,24,1.),1,0,1);
	    cp4.Draw1(ct);
	  ...
	  hpObservedYield.Write();
	  hpSignalYield.Write();
	  hpUnf.Write();
	  hpTrue2eBkg_clone.Write(fieldTrue2e);
	  hpFakeBkg_clone.Write(fieldFake);
	  hpSignalYield.Write();
	  hpCS.Write();
	  hTBValShape40->Write("h1TrueBkgValAt40");
	  hTBValShape41->Write("h1TrueBkgValAt41");
	  hTBValShape40tot->Write("h1TrueBkgTotalValAt40");
	  hTBValShape41tot->Write("h1TrueBkgTotalValAt41");
	  hValShape40tot->Write("h1SigYieldAt40");
	  hValShape41tot->Write("h1SigYieldAt41");
	...
	for (unsigned int i=0; res && (i<vecRnd.size()); ++i) {
	  ...
	  if (correctForNegativeValuesUnf) {
	...
	if (1) {
	  HERE("create unf distributions at 40 and 41");
	  TH1D* hUnfValAt40=Clone(hValShape40tot,"hUnfValShapeAt40");
	  TH1D* hUnfValAt41=Clone(hValShape41tot,"hUnfValShapeAt41");
	  for (unsigned int i=0; i<vecRnd.size(); ++i) {
	    const TH2D* h2=vecRnd[i];
	    hUnfValAt40->Fill(h2->GetBinContent(40,1));
	    hUnfValAt41->Fill(h2->GetBinContent(41,1));
	  }

	  HERE("create CS distributions at 40 and 41");
	  double dE=1e-2;
	  TH1D* hCSValShape40=new TH1D("hCSValShape40","hCSValShape40",50*multiplicity,-dE,dE);
	  TH1D* hCSValShape41=new TH1D("hValShape41","hCSValShape41",50*multiplicity,-dE/2.,dE/2.);
	  if (!hCSValShape40 || !hCSValShape41) {
	    std::cout << "null valShape\n";
	    return retCodeError;
	  }
	  hCSValShape40->Sumw2(); hCSValShape41->Sumw2();

	  for (unsigned int i=0; i<csRndV.size(); ++i) {
	    TH2D* h2=csRndV[i];
	    hCSValShape40->Fill(h2->GetBinContent(40,1));
	    hCSValShape41->Fill(h2->GetBinContent(41,1));
	  }

	  HERE("append study file");
	  TFile fStudy(fnameErrStudy,"update");
	  hUnfValAt40->Write("h1UnfValAt40");
	  hUnfValAt41->Write("h1UnfValAt41");
	  hCSValShape40->Write("h1CSValAt40");
	  hCSValShape41->Write("h1CSValAt41");
	  TObjString fnc40(errStudy_fnc40), fnc41(errStudy_fnc41);
	  TObjString par40(errStudy_pars40), par41(errStudy_pars41);
	  fnc40.Write("strFncAt40");
	  fnc41.Write("strFncAt41");
	  par40.Write("strParsAt40");
	  par41.Write("strParsAt41");
	  errStudyPar40.Write("vecParsAt40");
	  errStudyPar41.Write("vecParsAt41");
	  ...
	  HERE("draw CS event distributions at bins 40 and 41");
	  TCanvas *cv=new TCanvas("cv","cv",600,600);
	  ComparisonPlot_t cp3("cp3","cross section @40","#sigma","count","ratio");
	  cp3.SetRefIdx(-111);
	  removeError(hCSValShape40);
	  cp3.AddHist1D(hCSValShape40,"CS @40","LP", TAttMarker(kBlack,24,1.),1,0,1);
	  cp3.Draw1(cv);
