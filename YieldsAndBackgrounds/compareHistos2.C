...
void compareHistos2(TString conf, TString fName1, TString fName2) {
  ...
  for (unsigned int i=0; i<inpMgr.sampleCount(); ++i) {
    //if (i) break;
    TH1D *h1=(TH1D*)file1.Get(inpMgr.sampleName(i));
    TH2D *h2=(TH2D*)file2.Get(inpMgr.sampleName(i));

    if (!h1 || !h2) std::cout << "failed to get " << inpMgr.sampleName(i) << "\n";

    std::cout << "sample " << inpMgr.sampleName(i) << ": " << h1->GetName() << ", " << h2->GetName() << "\n";
    int ok=1;
	...
	double v2=h2->GetBinContent(ibin,1);
