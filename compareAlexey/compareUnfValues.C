...
int compareUnfValues(int analysisIs2D=0) {
  ...
  if (1) {
    TString fname="cmp_Alexey_unfYields.root";
    TFile fout(fname,"recreate");
    for (unsigned int i=0; i<hV.size(); ++i) {
      TString field=labelV[i];
      eliminateSeparationSigns(field);
      hV[i]->Write(TString("unfYields_") + field);
    }
    writeBinningArrays(fout,"compareAlexey/compareUnfValues",0);
    fout.Close();
