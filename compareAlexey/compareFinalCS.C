...
int compareFinalCS(int analysisIs2D=0, int saveCS=0) {
  ...
  if (bogusDetResp!=1) { // claimed response matrix
    int transposed=(bogusDetResp) ? 0 : 1;
  TH2D* detResp2raw=loadXYZTextFile("dir-Alexey-20140602/updated-response-matrix.txt","DetResponse_raw",0,transposed);
  if (!detResp2raw) return retCodeError;

  UnfM2=createMatrixD(detResp2raw,0);
  if (!UnfM2) return retCodeError;
  detResp2= createHisto2D(*UnfM2,NULL,"detResponse_Alexey","DetResponse_Alexey",_colrange_positive, 1,1.);
  ...
  if (0) {
    ...
    // earlier provided response matrix
    detResp2=LoadHisto2D("DetResponse","dir-Alexey/resMatrixProd.root","",0);
    if (!detResp2) return retCodeError;
    UnfM2=createMatrixD(detResp2,0);
    if (!UnfM2) return retCodeError;
  }
  ...
  if (1) {
    ...
    std::cout << " condition number ||DetResponse||*||DetResponseInv||=" << UnfM1->Norm1()*invUnf1.Norm1() << std::endl;
    std::cout << " chk ROOT bug: -condLU*||DetResponse||=" << (-condLU*UnfM1->Norm1()) << "\n" << std::endl;
    TMatrixD Mchk(*UnfM1);
    Mchk *= invUnf1;
    for (int ir=0; ir<Mchk.GetNrows(); ++ir) {
      for (int ic=0; ic<Mchk.GetNcols(); ++ic) {
	if (fabs(Mchk(ir,ic))<1e-14) Mchk(ir,ic)=0.;
      }
    }
    printMatrix("Mchk",Mchk);
  }
  ...
  if (1) {
    ...
    TH2D* h2UnfEffRhoYield2= Clone(h2UnfYield2_global,"h2UnfEffRhoYield_Alexey");
    ...
    TH2D* h2PostFsrYield2= Clone(h2UnfEffRhoYield2, "h2PostFsrYield_Alexey");
    ...
    TH2D* h2PostFsrCS2= Clone(h2PostFsrYield2, "h2PostFsrCS_Alexey");
    ...
    TH2D* h2PreFsrCS2= Clone(h2PostFsrCS2, "h2PreFsrCS_Alexey");
    ...
    if (saveCS) {
      TString fname="cmp_Alexey-20140603.root";
      if (bogusDetResp==1) fname.ReplaceAll(".root","-bogus.root");
      if (bogusDetResp==2) fname.ReplaceAll(".root","-transp.root");
      TFile fout(fname,"recreate");
      if (!saveHisto(fout,h2PreFsrCS1,"","") ||
	  !saveHisto(fout,h2PreFsrCS2,"","")) {
	return retCodeError;
      }
      writeBinningArrays(fout,"compareAlexey/compareFinalCS.C");
      fout.Close();
