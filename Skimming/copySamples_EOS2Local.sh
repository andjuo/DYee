...
#
# adapted from I.Kravchenko script
#

if [ ${#2} -eq 0 ] ; then
    echo "copySamples_EOS2Local.sh {zee|data} {skim|ntuple}"
    exit
fi
...
flist="./files_to_copy__${sample}_${skim}.txt"
echo
...
sourceDirEOS="/eos/cms/store/group/phys_smp/DYToEE_diff_xsec/ntuples/53X_with_regression/"
targetDirLocal="/tmp/andriusj/data/"

if [ ! -d ${targetDirLocal} ] ; then
    mkdir ${targetDirLocal}
fi
